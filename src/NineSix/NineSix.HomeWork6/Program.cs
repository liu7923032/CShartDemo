...
namespace NineSix.HomeWork6
{
    class Program
    {
        ...
        //加载价格
        private static CancellationTokenSource ctsPrice = new CancellationTokenSource();
        ...
        private static void InitDb(ICategoryService categoryService)
        {
            //1:初始化数据库
            categoryService.InitDbCatetory();
            categoryService.InitDbCommodity();
        }
        ...
        {
            //1:加载有URL请求的类别
            var categoryList = categoryService.GetAllList().Where(u => !string.IsNullOrEmpty(u.Url)).ToList();
